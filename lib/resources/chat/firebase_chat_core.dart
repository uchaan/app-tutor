...
class FirebaseChatCore {
  ...
  Future<types.Room> createRoom(
    ...
    final room = await FirebaseFirestore.instance
      ...
      'userRoles': null,
      'user1_uid': fu.uid,
      'user2_uid': otherUser.id,
      'user1_active': false,
      'user2_active': false,
      'user1_receivedNewMessage': false,
      'user2_receivedNewMessage': false,
    });
  ...
  Future<void> joinChat(
      Map<String, dynamic> otherUserInfo, BuildContext context) async {
    String otherUserId = otherUserInfo['uid'];
    print(otherUserInfo);

    Map FCMToken = otherUserInfo['FCMToken'];
    print(FCMToken);

    types.User otherUser = types.User(id: otherUserId);

    final room = await FirebaseChatCore.instance.createRoom(otherUser);

    Provider.of<UserState>(context, listen: false)
        .setChattingView(room, otherUserId, FCMToken);
    Provider.of<UserState>(context, listen: false).changeStateTo(CHATTING);

    // await Navigator.of(context).push(
    //   MaterialPageRoute(
    //     builder: (context) => ChatPage(
    //         room: room, FCMToken: FCMToken, name: name, uid: otherUserId),
    //   ),
    // );
    // newMessageList.remove(otherUserInfo['uid']);
  }
  ...
  void sendMessage(dynamic partialMessage, String roomId,
      Map<dynamic, dynamic> chatMap) async {
    ...
    if (message != null) {
      ...
      await FirebaseFirestore.instance
          ...
          .add(messageMap)
          .then((value) => _sendNotification(chatMap))
          .catchError((error) => print("Failed to send chat: $error"));
    }
  }

  Future<void> _sendNotification(Map<dynamic, dynamic> chatMap) async {
    print("_sendNotification");
    final response = await http
        .post(
      Uri.parse(pushServerUrl + '/push/chat-request'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode(chatMap),
    )
        .timeout(Duration(seconds: 3), onTimeout: () {
      return http.Response('Timeout Error', 500);
    });

    if (response.statusCode == 200) {
      print("Sending request successfully");
    } else if (response.statusCode == 500) {
      ///ToDO: 운영자에게 알릴 토스트 메세지 띄우기.
    } else {
      print('Fail to call');
    }
  }
